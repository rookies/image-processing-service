...
def upgrade():
    ...
    sa.Column('status', sa.Enum('WAITING', 'PROCESSING', 'FINISHED', 'FAILED', 'ABORTED', name='processingstatus'), nullable=False),
    sa.Column('output_uuid', ips.models.GUID(), nullable=True),
    sa.PrimaryKeyConstraint('uuid')
    )
    op.create_index(op.f('ix_processing_jobs_uuid'), 'processing_jobs', ['uuid'], unique=False)
